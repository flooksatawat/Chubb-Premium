...
window.render3DOptionsUI = function() {
    ...
    if (hxVal && hxOpts.includes(hxVal)) {
        ...
        html += `</div></div>`;
        html += `<div class="bg-white border border-slate-200/60 rounded-[14px] flex items-center justify-between mt-3" style="padding:6px 16px">`;
        html += `<button ontouchstart="window._hbfInterval=setInterval(()=>window.adjustHBF(-100),150)" ontouchend="clearInterval(window._hbfInterval)" onmousedown="window._hbfInterval=setInterval(()=>window.adjustHBF(-100),150)" onmouseup="clearInterval(window._hbfInterval)" onclick="window.adjustHBF(-100)" class="w-10 h-9 self-center flex items-center justify-center text-slate-400 hover:text-rose-600 rounded-[10px] hover:bg-slate-50 transition-colors select-none touch-manipulation"><i class="fas fa-minus text-[13px]"></i></button>`;
        html += `<input id="hbfCustomInput" type="number" inputmode="numeric" min="0" max="5000" step="100" value="${hbfNum === 0 ? '' : hbfNum}" placeholder="—" onchange="window.handle3DClick('HBF', Math.floor((parseInt(this.value)||0)/100)*100); this.blur();" class="flex-1 bg-transparent border-none text-center font-bold text-[16px] ${hbfNum === 0 ? 'text-slate-400' : 'text-rose-600'} outline-none p-0 [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none">`;
        html += `<button ontouchstart="window._hbfInterval=setInterval(()=>window.adjustHBF(100),150)" ontouchend="clearInterval(window._hbfInterval)" onmousedown="window._hbfInterval=setInterval(()=>window.adjustHBF(100),150)" onmouseup="clearInterval(window._hbfInterval)" onclick="window.adjustHBF(100)" class="w-10 h-9 self-center flex items-center justify-center text-slate-400 hover:text-rose-600 rounded-[10px] hover:bg-slate-50 transition-colors select-none touch-manipulation"><i class="fas fa-plus text-[13px]"></i></button>`;
        html += `</div></div>`;
    ...
    if (selOpen) {
        ...
        if (hxVal) {
            ...
            // HBF compact pill grid + stepper
            {
                ...
                stickyHtml += `</div>`;
                stickyHtml += `<div class="bg-white border border-slate-200/60 rounded-[14px] flex items-center justify-between" style="padding:4px 12px">
                    <button ontouchstart="window._hbfInterval=setInterval(()=>window.adjustHBF(-100),150)" ontouchend="clearInterval(window._hbfInterval)" onmousedown="window._hbfInterval=setInterval(()=>window.adjustHBF(-100),150)" onmouseup="clearInterval(window._hbfInterval)" onclick="window.adjustHBF(-100)" class="w-8 h-full flex items-center justify-center text-slate-400 hover:text-rose-600 rounded-[8px] hover:bg-slate-50 transition-colors select-none touch-manipulation"><i class="fas fa-minus text-[11px]"></i></button>
                    ...
                    <button ontouchstart="window._hbfInterval=setInterval(()=>window.adjustHBF(100),150)" ontouchend="clearInterval(window._hbfInterval)" onmousedown="window._hbfInterval=setInterval(()=>window.adjustHBF(100),150)" onmouseup="clearInterval(window._hbfInterval)" onclick="window.adjustHBF(100)" class="w-8 h-full flex items-center justify-center text-slate-400 hover:text-rose-600 rounded-[8px] hover:bg-slate-50 transition-colors select-none touch-manipulation"><i class="fas fa-plus text-[11px]"></i></button>
                </div></div>`;
            }
        }
        stickyHtml += `</div>`;
