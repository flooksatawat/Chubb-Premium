...
window.render3DOptionsUI = function() {
    ...
    if (hxVal && hxOpts.includes(hxVal)) {
        ...
        html += `</div></div>`;
        html += `<div class="premium-card !p-1.5 !px-4 !rounded-[14px] overflow-hidden flex items-center justify-between mt-3">`;
        html += `<button ontouchstart="window._hbfInterval=setInterval(()=>window.adjustHBF(-100),150)" ontouchend="clearInterval(window._hbfInterval)" onmousedown="window._hbfInterval=setInterval(()=>window.adjustHBF(-100),150)" onmouseup="clearInterval(window._hbfInterval)" onclick="window.adjustHBF(-100)" class="w-12 h-full flex items-center justify-center text-slate-400 hover:text-rose-600 rounded-[10px] hover:bg-slate-50 transition-colors select-none touch-manipulation"><i class="fas fa-minus text-[13px]"></i></button>`;
    ...
    if (selOpen) {
        ...
        if (hxVal) {
            ...
            // HBF compact pill grid + stepper
            {
                ...
                stickyHtml += `</div>`;
                stickyHtml += `<div class="premium-card !p-1 !rounded-[14px] flex items-center justify-between">
